...
func (c *NATSConsumer) handleVendorCreated(msg *nats.Msg) {
	...
	// If creation failed (for example, vendor already exists in billing), attempt update as fallback (upsert)
	if c.logger != nil {
...
func (c *NATSConsumer) handleVendorUpdated(msg *nats.Msg) {
	...
	// If vendor does not exist in billing yet, attempt creation as fallback
	if c.logger != nil {
